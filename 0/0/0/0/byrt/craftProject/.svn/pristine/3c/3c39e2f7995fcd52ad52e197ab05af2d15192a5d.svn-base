...
public class BytesUtil {
    ...
    /**
     * 将byte转换为一个长度为8的byte数组，数组每个值代表bit<p>
     * 顺序: bit0 -> bit8
     */
    public static byte[] bytToArrayBitAsc(byte b) {
     ...
     * 顺序: bit8 -> bit0
     */
    public static byte[] bytToArrayBit(byte b) {
        byte[] array = new byte[8];
        for (int i = 7; i >= 0; i--) {
            array[i] = (byte) (b & 1);
            b = (byte) (b >> 1);
     ...
     *
     * @param hexString the hex string 两位string一个byte
     * @return byte[]
     */
     ...
     * @return
     */
    public static int toInt2HighInFront(byte[] res, int begainIdx) {
        // 一个byte数据左移24位变成0x??000000，再右移8位变成0x00??0000
        int targets = (res[begainIdx + 1] & 0xff) | ((res[begainIdx] << 8) & 0xff00);
